...
props.globals.initNode("ai/control/object-selected","","STRING");
props.globals.initNode("ai/control/heading",0.00,"DOUBLE");
props.globals.initNode("ai/control/activecarrier",-1,"INT");
...
var Startup_yasim = func{
    ...
    if (getprop("sim/model/equipment/ground-services/external-power/enable") == 1) {
...
var Caution_panel = {
    ...
    right_bank: func{
        ...
        if(getprop("systems/electrical/outputs/caution-lights/low-press")>0.0)me.low_press.setValue(me.volts) else me.low_press.setValue(0);
        if(getprop("systems/electrical/outputs/caution-lights/reset-props")>0.0)me.prop_reset.setValue(me.volts) else me.prop_reset.setValue(0);
...
#        print("in range");
        ...
        var callsign = getprop("sim/mp-carriers/callsign");
...
#	        print("MP");
		...
		print(numplayers);
	        ...
	        {
            		var cpath = sprintf("ai/models/multiplayer[%u]/callsign",x);

			var carriername = getprop(cpath);

			if(carriername)
			{
	            		var idpath = sprintf("ai/models/multiplayer[%u]/id",x);
				var carrierid = getprop(idpath);

				if(carrierid > -1)
				{
					if(callsign == carriername) print(carriername);
#					print(carrierid);
	...
	else
	{
        ...
        var numcarriers = getprop("ai/models/count");
  
        for(var x=0;x<numcarriers;x=x+1)
        {
            var cpath = sprintf("ai/models/carrier[%u]/dlg-name",x);

            var carriername = getprop(cpath);
            if(carriername) {
#                print(carriername);
                setprop("ai/control/activecarrier",x);
                var hdgpath = sprintf("ai/models/carrier[%u]/orientation/true-heading-deg",x);
                var chdg = getprop(hdgpath);
...
setlistener("/controls/electric/master-avionics", func(test){
#     if(getprop("/controls/electric/power-source") != 0) (
         setprop("/systems/electrical/outputs/tacan",test.getValue());
#     )
},0,0);
